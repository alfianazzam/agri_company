...
<head>
    ...
    @vite(['assets/js/main.js', 'assets/css/style.css'])
...
<body data-spy="scroll" data-target=".onpage-navigation" data-offset="60">
    ...
    <main>
        ...
        {{-- Header --}}
        @if ($showHeader ?? true) <!-- Jika showHeader tidak didefinisikan, default ke true -->
            @include('components.navbar.index')
        @endif

        {{-- Jumbotron, jika ini adalah landing page, periksa kondisi --}}
        @if (Request::is('/'))
            @include('components.jumbotron.index')
        @endif

        {{-- Main Body Section --}}
        <div class="content">
            @yield('content') <!-- Ini akan diisi oleh child views -->
        </div>

        {{-- Footer Menu Section --}}
        @if ($showFooter ?? true) <!-- Jika showFooter tidak didefinisikan, default ke true -->
